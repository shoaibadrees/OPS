...
export class TableService {
  ...
  constructor(private _httpClient: HttpClient, private _global: AppService) {
    ...
    getAllTables(): Observable<ITableList[]> {//ITableList[]
  
    let url: string = this.apiUrl + 'HotelTableService/getAllTables' ;
        return this._httpClient.get<ITableList[]>(url).catch(this.handleError);
  }
    getAllTablesOnActive(): Observable<ITableList[]> { //ITableList[]
